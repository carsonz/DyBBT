...
def convert_msdialog_to_unified_format(domain: str, data_dir: str) -> Dict[str, List[Dict]]:
    ...
    # 加载数据
    user_goals = load_pickle_file(os.path.join(data_dir, 'user_goals_first.v1.p'))
    kb_data = load_pickle_file(os.path.join(data_dir, 'restaurant.kb.1k.v1.p'))
    
    # 读取slot信息
    with open(os.path.join(data_dir, 'restaurant_slots.txt'), 'r', encoding='utf-8') as f:
    ...
    ontology = {
        "domains": {
            "restaurant": {
                "description": "find and book restaurant reservations",
                "slots": {}
            }
        }
    }
    ...
    # 添加slots到ontology
    for slot in slots:
        ontology["domains"]["restaurant"]["slots"][slot] = {
            "description": f"{slot} information for restaurant reservation",
    ...
    for i, goal in enumerate(user_goals):
        dialogue_id = f"msdialog_restaurant_{i:04d}"
        
        # 构建goal
        goal_description = "You want to make a restaurant reservation. "
        ...
        turns.append({
            ...
            "state": {"restaurant": inform_slots}
        })
        
        dialogues.append({
            "dataset": "msdialog_restaurant",
            ...
            "goal": {
                ...
                "inform": {"restaurant": inform_slots},
                "request": {}
            },
            "turns": turns
        })
...
def main():
    """主函数"""
    print("开始处理MSDialog Restaurant数据集...")
    
    # 原始数据路径
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
    data_dir = os.path.join(base_dir, "EIERL", "src", "deep_dialog", "data_restaurant")
    
    # 转换数据
    dataset = convert_msdialog_to_unified_format("restaurant", data_dir)
    
    # 保存数据
    output_dir = os.path.dirname(__file__)
    
    with open(os.path.join(output_dir, 'dialogues.json'), 'w', encoding='utf-8') as f:
        json.dump({"train": dataset["train"], "validation": dataset["validation"], "test": dataset["test"]}, f, indent=2, ensure_ascii=False)
    ...
    # 创建shuffled IDs
    all_ids = [d["dialogue_id"] for d in dataset["train"] + dataset["validation"] + dataset["test"]]
    ...
    print(f"处理完成！训练集: {len(dataset['train'])}条, 验证集: {len(dataset['validation'])}条, 测试集: {len(dataset['test'])}条")
